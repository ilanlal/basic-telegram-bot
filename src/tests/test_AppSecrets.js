class Test_AppSecrets {
    constructor() {
        ...
        this.botToken = AspSecrets.BOT_TOKEN;
        this.chat_id = AspSecrets.ADMIN_CHAT_ID;
    ...
    test_ProjectSecrests() {
        QUnit.test("test secrets properties..", function (assert) {
            assert.ok(AspSecrets.BOT_TOKEN, "Bot token for test");
            assert.ok(AspSecrets.ADMIN_CHAT_ID, "Admin chat id for test");
            //assert.ok(AppSecrets.WEB_APP_URL, "Web app url for test");
            assert.ok(AspSecrets.DEPLOYMENT_ID, "Deployment id for test");
        });
    }
}
